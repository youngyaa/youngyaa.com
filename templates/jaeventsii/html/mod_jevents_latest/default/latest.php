...
class DefaultModLatestView
{
	...
	function displayLatestEvents()
	{
		...
		if (isset($this->eventsByRelDay) && count($this->eventsByRelDay))
		{
			$content .= $this->modparams->get("modlatest_templatetop") ? $this->modparams->get("modlatest_templatetop") : '<div class="mod_events_latest_table jevbootstrap row">';
			...
			foreach ($this->eventsByRelDay as $relDay => $daysEvents)
			{
				...
				foreach ($daysEvents as $dayEvent)
				{
					...
					if ($firstTime)
						$eventrow = '<div class="mod_events_latest mod_events_latest_first"><div class="inner">%s'."</div></div>\n";
					elseif ($i%4==0)
						$eventrow = '<div class="mod_events_latest mod_events_latest_firstrow_lg"><div class="inner">%s'."</div></div>\n";
					else 
						$eventrow = '<div class="mod_events_latest"><div class="inner">%s'."</div></div>\n";
		...
		{
			$content .= $this->modparams->get("modlatest_templatetop") ? $this->modparams->get("modlatest_templatetop") : '<div class="mod_events_latest_table jevbootstrap row" >';
			$templaterow = $this->modparams->get("modlatest_templaterow") ? $this->modparams->get("modlatest_templaterow")  : '<div class="mod_events_latest_noevents">%s</div>' . "\n";
		...
		(function ($) {
		  $(document).ready(function(){ 
		    $(".t3-section .mod_events_latest_table.jevbootstrap").owlCarousel({
		      navigation : true,
		      pagination: false,
		      items: 4,
		      loop: false,
