...
class Bt_mediaViewList extends JViewLegacy {
    ...
    public function display($tpl = null) {
        ...
        if ($this->_layout == 'default') {
            if ($this->legacy) {
                ...
                Bt_mediaHelper::addSubmenu('list');
                $this->addToolbar();
    ...
    protected function addToolbar() {
        ...
        JToolBarHelper::title(JText::_('COM_BT_MEDIA_MENU_MEDIASMANAGEMENT_TITLE'), 'list.png');
        ...
        if (file_exists($formPath)) {
            ...
            if ($canDo->get('core.create')) {
                ...
                if ($canGetImage || $canUploadImage || $canGetVideo || $canUploadVideo) {
                    JToolBarHelper::custom('detail.multiadd', 'multiadd.png', 'multiadd.png', 'Multiple Add', FALSE);
                }
            }
            JToolBarHelper::custom('list.rebuild', 'refresh.png', 'refresh_f2.png', 'COM_BT_MEDIA_MEDIAMANAGEMENT_REBUILD_IMAGES', false);
        ...
        if (!$this->legacy) {
            JHtmlSidebar::setAction('index.php?option=com_bt_media&view=list');
            ...
            JHtmlSidebar::addFilter(
                    JText::_('COM_BT_MEDIA_CATEGORY_SELECT'), 'filter_cate_id', JHtml::_('select.options', Bt_mediaLegacyHelper::getCategoryOptions(), 'value', 'text', $this->state->get('filter.cate_id'))
            );
            ...
            JHtmlSidebar::addFilter(
                    JText::_('COM_BT_MEDIA_ITEMS_MEDIASMANAGEMENT_MEDIA_TYPE_FILTER'), 'filter_media_type', JHtml::_('select.options', $media_type, "value", "text", $this->state->get('filter.media_type'), true)
            );
